    ...
    <nav className="fixed top-0 left-0 right-0 z-50 backdrop-blur-xl bg-background/70 border-b border-border/40 supports-[backdrop-filter]:bg-background/60 transition-all duration-300">
      <div className="container mx-auto flex items-center justify-between px-6 py-4">
        <div className="flex items-center gap-3">
          <a href="https://tanvir-chowdhury.github.io/tanvir-portfolio/" className="flex items-center gap-2.5 group">
            <div className="w-9 h-9 rounded-xl bg-foreground text-background flex items-center justify-center font-display font-bold text-sm group-hover:scale-105 transition-transform duration-300">
              TC
            </div>
            <span className="font-display font-semibold text-base tracking-tight group-hover:text-primary transition-colors hidden sm:inline">
              Tanvir Chowdhury
            </span>
          </a>
        </div>
        ...
        <div className="hidden lg:flex items-center gap-8">
          {['About Me', 'Projects', 'Education', 'Experience', 'Certificates', 'Awards', 'Skills', 'Hobbies', 'Contact'].map((item) => (
            <a
              ...
              className="text-sm font-medium text-muted-foreground hover:text-foreground transition-colors relative after:content-[''] after:absolute after:left-0 after:bottom-[-4px] after:h-[2px] after:w-0 after:bg-primary after:transition-all after:duration-300 hover:after:w-full"
            >
              {item}
            </a>
          ))}
          <a href="mailto:tanvir.chowdhury.us@gmail.com">
            <Button size="sm" className="ml-4 shadow-md shadow-primary/20 rounded-full px-6">
              Hire me
            </Button>
          </a>
          <div className="ml-2">
      ...
      {open && (
        <div className="lg:hidden border-t border-border/40 bg-background/95 backdrop-blur-xl absolute w-full left-0 animate-in slide-in-from-top-5 fade-in duration-200">
          <div className="container mx-auto px-6 py-6 flex flex-col gap-4">
            ...
            <Button size="lg" asChild className="mt-4 w-full rounded-xl shadow-lg shadow-primary/20">
